...
import os, sys, re, json, uuid, asyncio, datetime, atexit, webbrowser
...
def _write_mermaid_bundle(compiled_graph, session_id: str) -> Optional[str]:
    """
    Emit a .mmd and a self-contained HTML viewer, open HTML in browser.
    """
    try:
    ...
    mmd_path = Path(f"graph_{session_id}.mmd")
    mmd_path.write_text(mermaid, encoding="utf-8")
    print(f"Mermaid saved: {mmd_path.name}")
...
<head>
  ...
  <style>body{{margin:0;padding:16px;font-family:system-ui;}} .wrap{{max-width:1200px;margin:auto;}}</style>
</head>
<body>
<div class="wrap">
...
def load_context(choice: str) -> tuple[str, dict]:
    """Defer to lkk’s loader to keep behavior identical (no hardcoding)."""
...
# We support both import paths; if neither works, we still run without Studio.
_STUDIO_AVAILABLE = False
try:
    from langgraph.checkpoint.sqlite import SqliteSaver          # langgraph>=0.2 style
    _STUDIO_AVAILABLE = True
except Exception:
    try:
        from langgraph.checkpoint import SqliteSaver             # some distros ship it here
        _STUDIO_AVAILABLE = True
    except Exception:
...
def build_app():
    """
    Exposes `app` for `langgraph dev` (Studio UI). Safe even if SqliteSaver missing.
    Run:  langgraph dev  (then open the local URL it prints)
    """
    compiled = _build_compiled_graph()
    if _STUDIO_AVAILABLE:
        try:
            # Plain file path accepted in newer versions; use DSN if required by your build.
            checkpointer = SqliteSaver.from_conn_string("ui_checkpoints.sqlite")
            compiled = compiled.with_config(checkpointer=checkpointer)  # attach persistence
        except Exception:
            pass
    return compiled
...
async def _run_with_events(compiled_graph, initial, recursion_limit: int, session_id: str):
    """
    Stream LangGraph events to terminal and save both TXT and JSONL timelines.
    ...
    with open(timeline_path_txt, "w", encoding="utf-8") as log_txt, \
        ...
        try:
            async for ev in compiled_graph.astream_events(
                ...
                # JSONL (sanitize non-serializable objects)
                def _safe(obj):
                    try:
                        json.dumps(obj)
                        return obj
    ...
    if final_state is None:
        # Fallback in case the stream didn't include the final output
...
async def generate_podcast(topic: Optional[str] = None,
    ...
    # Mermaid export + auto-open HTML viewer
    _write_mermaid_bundle(compiled_graph, session_id)
